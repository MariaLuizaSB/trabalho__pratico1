            ...
            <div class="flex-items2">
                <form method="POST"  action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
                    ...
                    if ($_SERVER["REQUEST_METHOD"] == "POST"){
                        ...
                        $inputCpf = $_POST["cpf"];
                        $inputDatanascimento = $_POST["dataNasc"];
                        $inputTelefone = $_POST["telefone"];
                        $inputEmail = $_POST["email"];
                        $inputAluno = isset($_POST["input-aluno"])? "sim" : "não";
                        $dataNascimento = new DateTime($inputDatanascimento);
                        $dataAtual = new DateTime(date('d-m-Y'));
                        $intervalo = $dataAtual->diff($dataNascimento);

                        $idade = $intervalo->y;

                        echo "Eu $inputNome, $inputAluno sou estudante. Meu número de CPF é $inputCpf, nasci em " . $dataNascimento->format('d/m/Y') . " e tenho $idade.
                        Meu telefone de contato é $inputTelefone e o meu e-mail é $inputEmail";

                    } else if ($_SERVER["REQUEST_METHOD"] == "GET" && !empty($_POST)) {
